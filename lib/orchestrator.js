...
const getTrimmedMemory = (limit = 6) => memory.slice(-limit);
...
async function callDobby(systemPrompt, userPrompt, retries = 1) {
  ...
  try {
    ...
    console.log(`🔥 Fireworks [${systemPrompt.slice(0, 25)}...] →`, text.slice(0, 100), "...");
    return text;
  } catch (err) {
...
export async function runAgents(prompt, streamLog) {
  ...
  memory = [...getTrimmedMemory(10), prompt, final];
  ...
  return {
    final,
    agents: completed.map((a) => ({ role: a.role, output: a.output })),
  };
}
